import { pipe, isObservable, from as from$1, of as of$1, Subject as Subject$1, EMPTY as EMPTY$1, NEVER, combineLatest } from 'rxjs';
...
const catchErr = pipe(catchError(e => of$1(e)));
...
function action(type, config) {
    if (typeof config === "function") {
        return defineType(type, function () {
            ...
            return (__assign(__assign({}, config.apply(void 0, args)), { type: type }));
        });
    }
        ...
        case "fsa":
            return defineType(type, function (payload, meta) {
                return payload instanceof Error
                    ...
                    : { error: false, meta: meta, payload: payload, type: type };
            });
        case "payload":
            return defineType(type, function (payload) { return ({ payload: payload, type: type }); });
        case "props":
...
function union() {
    ...
    var result = creators.map(function (_a) {
        var type = _a.type;
        return Object.defineProperty({ type: type }, "action", descriptor);
    });
...
function isType(action) {
    var creators = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        creators[_i - 1] = arguments[_i];
    }
    return creators.some(function (_a) {
        var type = _a.type;
...
function guard() {
    ...
    return (function (action) { return isType.apply(void 0, __spreadArrays([action], creators)); });
}
...
function on() {
    ...
    var types = args.reduce(function (result, creator) { return __spreadArrays$1(result, [creator.type]); }, []);
...
function getDefaults(config = {}, options = {}) {
    const reducer$ = (config && config.reducer$ && config.reducer$.pipe(catchErr)) ||
        of$1(reducer({}));
    const actions$ = new Subject$1();
    const actionStream$ = reducer$ => ((config && config.actionStream$ && config.actionStream$.pipe(catchErr)) ||
        EMPTY$1).pipe(filter(isObject$1), map(mapToObservable), actionFlatten(flatCatch), tap(actions$), reducer$, map(mapToObservable));
...
export { FlattenOperator, Store, action, catchErr, createSelector, createSelectorCreator, createStore, createStructuredSelector, defaultMemoize, empty$2 as empty, filterA, filterNS, filterPS, fsa, guard, isType, mapA, mapNS, mapPS, mapToObservable, on, payload, props, reduceA, reduceNS, reducePS, reducer, type, union };
